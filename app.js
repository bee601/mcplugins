...
const state = { search: '', sources: new Set(), platforms: new Set(), prices: new Set(), version: 'all', quick: 'all', sort: 'relevance', saved: new Set(), view: 'catalog' };
...
function buildFilterOptions() {
  const options = { sourceFilters: ['Modrinth', 'Spigot'], platformFilters: ['Paper', 'Spigot', 'Fabric', 'Quilt', 'Velocity'], priceFilters: ['Free', 'Paid'] };
...
async function fetchSpigot(page) {
  ...
  const data = await response.json();
  return data.map((item) => ({ id: `spigot-${item.id}`, name: item.name, creator: item.author?.name || 'Unknown creator', source: 'Spigot', sources: ['Spigot'], platform: ['Spigot', 'Paper'], versions: (item.testedVersions || []).map((version) => version.split('.').slice(0, 2).join('.')), price: item.premium ? 'Paid' : 'Free', icon: (item.name || 'S')[0].toUpperCase(), iconUrl: item.icon?.url ? `https://www.spigotmc.org/${item.icon.url}` : '', iconClass: 'icon-blue', category: item.tag || 'Minecraft plugin', downloads: formatNumber(item.downloads || 0), downloadValue: item.downloads || 0, downloadSources: [{ source: 'Spigot', value: item.downloads || 0 }], updated: relativeDate(item.updateDate), age: ageInDays(item.updateDate), popularity: item.rating?.average || item.downloads || 0, description: stripHtml(item.description) || 'Minecraft plugin from the Spigot resource index.', url: `https://www.spigotmc.org/resources/${item.id}/`, links: [{ label: 'Spigot resource', url: `https://www.spigotmc.org/resources/${item.id}/` }, item.links?.R2l0aHVi && { label: 'GitHub', url: item.links.R2l0aHVi }, item.links?.discussion && { label: 'Discussion', url: item.links.discussion }].filter(Boolean) }));
}
...
function getFilteredPlugins() {
  ...
  let list = plugins.filter((plugin) => {
    ...
    return matchesSearch && matchesSource && matchesPlatform && matchesPrice && matchesVersion && matchesQuick;
  });
...
function render() {
  ...
  $('#filterCount').textContent = state.sources.size + state.platforms.size + state.prices.size + (state.version !== 'all' ? 1 : 0);
...
function pluginCard(plugin) { const saved = state.saved.has(plugin.id); const icon = plugin.iconUrl ? `<img src="${escapeHtml(plugin.iconUrl)}" alt="" loading="lazy" onerror="this.hidden=true;this.nextElementSibling.hidden=false" /><span hidden>${plugin.icon}</span>` : `<span>${plugin.icon}</span>`; const downloadText = (plugin.downloadSources || []).map((entry) => `${entry.source}: ${formatNumber(entry.value)}`).join(' · '); return `<article class="plugin-card" data-plugin="${plugin.id}"><div class="card-top"><span class="plugin-icon ${plugin.iconClass}">${icon}</span><div class="card-actions"><button data-save="${plugin.id}" class="${saved ? 'saved' : ''}" title="${saved ? 'Remove from saved' : 'Save plugin'}" aria-label="${saved ? 'Remove from saved' : 'Save plugin'}"><i data-lucide="bookmark"></i></button><button title="Open plugin details" aria-label="Open plugin details"><i data-lucide="arrow-up-right"></i></button></div></div><h3>${escapeHtml(plugin.name)}</h3><p class="description">${escapeHtml(plugin.description)}</p><div class="meta-row"><span title="${escapeHtml(downloadText)}"><i data-lucide="download"></i>${escapeHtml(downloadText)}</span><span><i data-lucide="clock-3"></i>${plugin.updated}</span></div><div class="card-bottom"><span class="source-name"><i class="source-dot ${(plugin.sources?.[0] || plugin.source).toLowerCase()}"></i>${plugin.source}</span><span class="price-tag ${plugin.price === 'Free' ? 'free' : 'paid'}">${plugin.price}</span></div></article>`; }
...
function clearFilters() { state.sources.clear(); state.platforms.clear(); state.prices.clear(); state.version = 'all'; state.quick = 'all'; $('#versionFilter').value = 'all'; $$('.chip').forEach((item) => item.classList.toggle('active', item.dataset.quick === 'all')); $$('.source-tab').forEach((item) => item.classList.toggle('active', item.dataset.source === 'all')); syncCheckboxes(); loadPlugins(true); }
...
function openDrawer(id) { const plugin = plugins.find((item) => item.id === id); if (!plugin) return; const icon = plugin.iconUrl ? `<img src="${escapeHtml(plugin.iconUrl)}" alt="" onerror="this.hidden=true;this.nextElementSibling.hidden=false" /><span hidden>${plugin.icon}</span>` : plugin.icon; const links = (plugin.links || []).map((link) => `<a href="${escapeHtml(link.url)}" target="_blank" rel="noreferrer"><i data-lucide="external-link"></i>${escapeHtml(link.label)}</a>`).join(''); $('#drawerContent').innerHTML = `<div class="drawer-hero"><span class="plugin-icon ${plugin.iconClass}">${icon}</span><h2 id="drawerTitle">${escapeHtml(plugin.name)}</h2><p>${escapeHtml(plugin.description)}</p></div><div class="drawer-details"><div class="detail-item"><span>CREATOR</span><strong>${escapeHtml(plugin.creator)}</strong></div><div class="detail-item"><span>SOURCE</span><strong>${plugin.source}</strong></div><div class="detail-item"><span>PLATFORM</span><strong>${plugin.platform.join(', ')}</strong></div><div class="detail-item"><span>VERSION</span><strong>${plugin.versions.length ? plugin.versions.map((version) => `${version}.x`).join(', ') : 'Not listed'}</strong></div><div class="detail-item"><span>DOWNLOADS</span><strong>${plugin.downloads}</strong></div><div class="detail-item"><span>PRICE</span><strong>${plugin.price}</strong></div></div><div class="download-row"><select id="downloadMode"><option value="open">Open official download page</option><option value="copy">Copy download link</option></select><button class="drawer-cta" id="downloadAction"><i data-lucide="download"></i> Download</button></div><div class="drawer-links"><span>PROJECT LINKS</span>${links || '<small>No additional links listed</small>'}</div><div class="drawer-note"><i data-lucide="info"></i><span>Your browser controls the local download folder. The link always points to the original marketplace.</span></div>`; $('#drawerBackdrop').hidden = false; $('#detailsDrawer').classList.add('open'); $('#detailsDrawer').setAttribute('aria-hidden', 'false'); if (window.lucide) lucide.createIcons(); $('#downloadAction').addEventListener('click', () => { if ($('#downloadMode').value === 'copy') { navigator.clipboard?.writeText(plugin.url); toast('Download link copied'); } else { window.open(plugin.url, '_blank', 'noopener'); } }); }
